...
export class DataController {
  ...
  async openContact(
    ...
    try {
      ...
      if (deleteResult.affected > 0) {
        await this.dataService.addStory({
          hanhdong: `Admin ${user.admin.HOTEN} đã mở trạng thái liên hệ ${body.TRANGTHAILIENHE} cho đoạn được chọn`,
          maadmin: user.MAADMIN,
          sdt: null,
        });
      }
  ...
  @Get('/segment')
  async getOneSegmentDetail(@Query() query: {}, @Res() res: Response) {
    try {
      const data = await this.dataService.getDataSegment(query);
